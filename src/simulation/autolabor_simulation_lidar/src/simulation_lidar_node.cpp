...
#include "autolabor_simulation_stage/ObstacleMap.h"
...
namespace autolabor_simulation {
    ...
    SimulationLidar::SimulationLidar() {
        ...
        private_node.param("map_service_name", map_service_name_, string("/map_handle"));

        private_node.param("use_map_service", use_map_service_, false);
        XmlRpc::XmlRpcValue xml_ignore_object_id;
        if (private_node.hasParam("ignore_object_id") &&
            private_node.getParam("ignore_object_id", xml_ignore_object_id)) {
            ROS_ASSERT(xml_ignore_object_id.getType() == XmlRpc::XmlRpcValue::TypeArray);
            for (int i = 0; i < xml_ignore_object_id.size(); i++) {
                ROS_ASSERT(xml_ignore_object_id[i].getType() == XmlRpc::XmlRpcValue::TypeString);
                std::cout << static_cast<std::string>(xml_ignore_object_id[i]) << std::endl;
                ignore_object_id_.push_back(static_cast<std::string>(xml_ignore_object_id[i]));
            }
        }
    ...
    void SimulationLidar::distance(int start_x, int start_y, double theta, int &end_x, int &end_y) {
        ...
        if (limit_range(start_x, start_y)) {
            ...
            while (true) {
                ...
                if (limit_range(start_x, start_y) || getValue(start_x, start_y) == OBS_SPACE ||
                    getValue(start_x, start_y) == UNKNOWN_SPACE) {
                    end_x = start_x;
                    end_y = start_y;
                    return;
                }
    ...
    void SimulationLidar::pubLaserCallback(const ros::TimerEvent &event) {
        ...
        map_mutex_.lock();
        if (use_map_service_) {
            autolabor_simulation_stage::ObstacleMap msg;
            msg.request.ignore_obstacle_id = ignore_object_id_;
            if (client_.call(msg)) {
                local_map_ = msg.response.obstacle_map;
                data_length_ = local_map_.info.width * local_map_.info.height;
            }
        }
        getFrame(laserscan);
        map_mutex_.unlock();
    ...
    void SimulationLidar::run() {
        ...
        if (use_map_service_) {
            client_ = nh_.serviceClient<autolabor_simulation_stage::ObstacleMap >(map_service_name_);
        } else {
