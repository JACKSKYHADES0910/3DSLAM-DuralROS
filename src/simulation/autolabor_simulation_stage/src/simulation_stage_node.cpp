...
namespace autolabor_simulation {
    ...
    SimulationStage::SimulationStage() : exist_obstacle_(false), data_length_(0) {
    ...
    char SimulationStage::calValue(int index, char *obs_data) {
        if (index < 0 && index > data_length_) {
            return ERROR;
        }
        if (exist_obstacle_) {
            return obs_data[index] == OBS_SPACE ? OBS_SPACE : map_data_[index];
        } else {
            return map_data_[index];
        }
    }
    ...
    void SimulationStage::mapReceived(const nav_msgs::OccupancyGrid::ConstPtr &grid_map) {
        ...
        tmp_obstacle_data_ = new char[data_length_];
        for (int i = 0; i < data_length_; i++) {
    ...
    void
    SimulationStage::getObstacleRange(footprint &footprint, float &min_x, float &max_x, float &min_y, float &max_y) {
        if (footprint.size() > 0) {
    ...
    bool SimulationStage::pnpoly(footprint &footprint, float &x, float &y) {
        ...
        for (i = 0, j = footprint.size() - 1; i < footprint.size(); j = i++) {
            if (((footprint.at(i).y > y) != (footprint.at(j).y > y)) &&
                (x < (footprint.at(j).x - footprint.at(i).x) * (y - footprint.at(i).y) /
                     (footprint.at(j).y - footprint.at(i).y) + footprint.at(i).x)) {
                c = !c;
            }
        }
        return c;
    ...
    void
    SimulationStage::insertObstacle(string obstacle_name, vector<geometry_msgs::Polygon> polygon_list, float x, float y,
                                    float yaw) {
    ...
    void SimulationStage::updateObstacleData() {
        ...
        for (obstacle_points_map::iterator iter = obstacle_points_map_.begin();
             iter != obstacle_points_map_.end(); iter++) {
    ...
    bool SimulationStage::obstacleHandleServer(autolabor_simulation_stage::Obstacle::Request &req,
                                               autolabor_simulation_stage::Obstacle::Response &res) {
    ...
    bool SimulationStage::mapHandleServer(autolabor_simulation_stage::ObstacleMap::Request &req,
                                          autolabor_simulation_stage::ObstacleMap::Response &res) {
        int mx, my, index;
        memset(tmp_obstacle_data_, 0, data_length_);
        for (auto & iter : obstacle_points_map_) {
            if (find(req.ignore_obstacle_id.begin(), req.ignore_obstacle_id.end(), iter.first) !=
                req.ignore_obstacle_id.end()) {
                continue;
            }
            ...
            for (int i = 0; i < (iter.second).size(); i++) {
                ...
                index = my * size_x_ + mx;
                if (index >= 0 && index <= data_length_) {
                    tmp_obstacle_data_[index] = OBS_SPACE;
                }
            }
        }

        nav_msgs::OccupancyGrid res_map_msg;
        ros::Time current_time = ros::Time::now();
        res_map_msg.header.stamp = current_time;
        res_map_msg.header.frame_id = real_map_frame_;
        res_map_msg.info.map_load_time = current_time;
        res_map_msg.info.resolution = resolution_;
        res_map_msg.info.width = size_x_;
        res_map_msg.info.height = size_y_;
        res_map_msg.info.origin.position.x = origin_x_;
        res_map_msg.info.origin.position.y = origin_y_;
        res_map_msg.info.origin.orientation.w = 1.0;
        for (int i = 0; i < data_length_; i++) {
            res_map_msg.data.push_back(calValue(i, tmp_obstacle_data_));
        }
        res.obstacle_map = res_map_msg;
    ...
    void SimulationStage::run() {
        ...
        map_service_ = nh_.advertiseService("map_handle", &SimulationStage::mapHandleServer, this);
        ros::spin();
    }
