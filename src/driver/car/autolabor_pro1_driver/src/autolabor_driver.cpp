...
#include "autolabor_pro1_driver/AutolaborModelConfig.h"
...
private:
    ...
    void config_callback(autolabor_pro1_driver::AutolaborModelConfig &config, uint32_t level);
...
private:
    ...
    dynamic_reconfigure::Server<autolabor_pro1_driver::AutolaborModelConfig> server_;
    dynamic_reconfigure::Server<autolabor_pro1_driver::AutolaborModelConfig>::CallbackType f_;
...
void ChassisDriver::config_callback(autolabor_pro1_driver::AutolaborModelConfig &config, uint32_t level) {
    model_param_acw_ = config.model_param_acw;
    model_param_cw_ = config.model_param_cw;
    wheel_diameter_ = config.wheel_diameter;
...
void ChassisDriver::handle_speed_msg(uint8_t *buffer_data) {
    ...
    ROS_DEBUG_STREAM("receive -> left: " << delta_left_ << "(" << rev_left_ << ")" << "; right: " << delta_right_ << "("
                                         << rev_right_ << ")");
    ...
    if (delta_time_ >= (0.5 / control_rate_)) {
        ...
        ROS_DEBUG_STREAM("accumulation_x: " << accumulation_x_ << "; accumulation_y: " << accumulation_y_
                                            << "; accumulation_th: " << accumulation_th_);
    }
    last_time_ = now_;
}
...
void ChassisDriver::run() {
    ...
    if (init()) {
        ...
        f_ = boost::bind(&ChassisDriver::config_callback, this, _1, _2);
        server_.setCallback(f_);

        ros::Subscriber cmd_sub = node.subscribe<geometry_msgs::Twist>("/cmd_vel", 10, &ChassisDriver::twist_callback,
                                                                       this);
        ros::Timer send_speed_timer = node.createTimer(ros::Duration(1.0 / control_rate_),
                                                       &ChassisDriver::send_speed_callback, this);
        ros::Timer ask_battery_remainder_timer = node.createTimer(ros::Duration(1.0 / sensor_rate_),
                                                                  &ChassisDriver::ask_battery_remainder_callback, this);
        ros::Timer ask_current_timer = node.createTimer(ros::Duration(1.0 / sensor_rate_),
                                                        &ChassisDriver::ask_current_callback, this);
        ros::Timer ask_voltage_timer = node.createTimer(ros::Duration(1.0 / sensor_rate_),
                                                        &ChassisDriver::ask_voltage_callback, this);
