...
int main(int argc, char **argv) {
  ...
  nh_private.param("enable_angle_keep", setting.enable_angle_keep, bool(false));
  nh_private.param("angle_keep_min", setting.angle_keep_min, double(0.0));
  nh_private.param("angle_keep_max", setting.angle_keep_max, double(0.0));
...
void  ToLaserscanMessagePublish(ldlidar::Points2D& src, ldlidar::LiPkg* commpkg, LaserScanSetting& setting, ros::Publisher& lidarpub) {
  ...
  if (commpkg->GetSpeedOrigin() > 0) {
    ...
    for (auto point : src) {
      ...
      if (setting.enable_angle_keep)
      {
          if (dir_angle < setting.angle_keep_min || dir_angle > setting.angle_keep_max)
          {
              range = std::numeric_limits<float>::quiet_NaN();
              intensity = std::numeric_limits<float>::quiet_NaN();
          }
